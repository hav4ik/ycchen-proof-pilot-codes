...
|---|---|---|---|---|
| `chankhavu/ycchen-opd:cu128` | **`sha256:9d36b998…`** | `dcbe556` | fixes #1–#10 + `JIT_CACHE_DIR` + `faf4c62` + seed mirror + **B200 teacher `flashinfer_mxfp4` auto-detect** + harness fix + prefill bench | **PRODUCTION / SHIP** — drift-clean (`dcbe556..HEAD` empty) |
| `chankhavu/ycchen-opd:cu128` (intermediate) | `sha256:451201a8…` | `94efb6c` | missing the B200 teacher MoE fixes (`88dce35`/`dcbe556`/`8240b78`) | superseded — do NOT ship |
...
- **⚠️ IMAGE↔SOURCE DRIFT (the bug that bit us — now RESOLVED):** a pushed image lags the source. `5e3ba5f6` was
  ...
  `451201a8`@`94efb6c` baked `faf4c62`, then the final `9d36b998`@`dcbe556` baked the B200 teacher fixes — drift
  check `dcbe556..HEAD` empty. **`9d36b998` is the ship image.** Full gate: `OPD_V2_SHIP_CHECKLIST.md`.
...
- **agentic `max_prompt_tokens > max_traj` guard:** config default 100000 is sized for her 130k prod ctx; the
  ...
  MAX_TRAJ/2 (= 28672) via `faf4c62`, **now baked in the ship image `9d36b998`** (no manual override needed;
...
1. ✅ **Final drift-clean rebuild — DONE** → **`sha256:9d36b998…`** (built at `dcbe556`). Baked `8240b78`
   (harness) + `88dce35` (teacher MoE env) + `99b2d03` (bench) + `dcbe556` (auto-detect `flashinfer_mxfp4` on
   sm_100). Drift check `dcbe556..HEAD` EMPTY; fixes spot-checked in-container. **This is the ship image for Ai2.**
